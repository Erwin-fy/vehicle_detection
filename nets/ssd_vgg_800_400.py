...
class SSDNet(object):
    """Implementation of the SSD VGG-based 800*400 network.

    The default features layers with 800x400 image input are:
      conv4 ==> 38 x 38
      conv7 ==> 19 x 19
      conv8 ==> 10 x 10
      conv9 ==> 5 x 5
    ...
    default_params = SSDParams(
        ...
        feat_shapes=[(50, 100), (25, 50), (13, 25), (7, 13), (4, 7), (1, 1)],
        ...
        anchor_ratios=[[2, .5],
                       ...
                       [2, .5],
                       [2, .5]],
        anchor_steps=[8, 16, 32, 64, 100, 400],
        anchor_offset=0.5,
...
def ssd_net(inputs,
    ...
    end_points = {}
    with tf.variable_scope(scope, 'ssd_800_400_vgg', [inputs], reuse=reuse):
        # Original VGG-16 blocks.
        print inputs.op.name, inputs.get_shape().as_list()
        ...
        with tf.variable_scope(end_point):
            ...
            print net.op.name, net.get_shape().as_list()
            '''
            net = slim.avg_pool2d(net, [4, 7], stride=1, padding='VALID')
            print net.op.name, net.get_shape().as_list()
        end_points[end_point] = net
...
ssd_net.default_image_size = 400
...
def ssd_losses(logits, localisations,
    ...
    with tf.name_scope(scope, 'ssd_losses'):
        print np.shape(localisations), np.shape(glocalisations), np.shape(logits)
        ...
        for i in range(len(logits)):
            print i, logits[i].get_shape().as_list(), gclasses[i].get_shape().as_list(), gscores[i].get_shape().as_list(), localisations[i].get_shape().as_list(), glocalisations[i].get_shape().as_list()
        ...
        print localisations.get_shape().as_list(), glocalisations.get_shape().as_list(), logits.get_shape().as_list()
        ...
        print localisations.get_shape().as_list(), glocalisations.get_shape().as_list()
